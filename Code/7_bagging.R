...
for (b in 1:B) {
  ...
  pred_mat[, b] <- predict(fit_b, newdata = test) #Uses the bth tree to predict on the test data (te).
}
...
# ==========================
p <- ncol(tr) - 1  # number of predictors (educ, exper, tenure)
...
pred_te_rf <- predict(rf_bag, newdata = te)
rmse_rf    <- rmse(te$wage, pred_te_rf)
